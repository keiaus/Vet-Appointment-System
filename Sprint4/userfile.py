...
class User():
    ...
    def user_searchvet(self):
        ...
        displayvetsframe = Frame(self.user_searchvet_frame)
        displayvetsframe.pack(side = "right")
        ...
        #LABELFRAME (INCOMPLETE CODE, DOESN'T WORK YET)
        def display_vets():
            my_data = cursor.execute(f"""select Concat('Forename: ', VetAccountInfo.VetFirstName, ' Surname: ', VetAccountInfo.VetLastName, 
                                    ' Phone: ', VetAccountInfo.VetPhoneNumber, ' Email: ', VetAccountInfo.VetEmailAddress,' Street: ', 
                                    VetAccountInfo.VetStreetAddress, ' City: ', VetAccountInfo.VetCity, ' State: ', VetAccountInfo.VetState, 
                                    ' Zip: ', VetAccountInfo.VetZip, ' Specialization: ', VetAccountInfo.Specialization, ' Day/Time: ', 
                                    VetScheduleInfo.{day_cb_get}) from VetAccountInfo join VetScheduleInfo on VetScheduleInfo.VetID = VetAccountInfo.VetID 
                                    where VetAccountInfo.Specialization = ? and VetAccountInfo.VetZip = ? and VetScheduleInfo.{day_cb_get} is not null", special_cb_get, zipcode_entry_get""")
            ...
            for i in range(len(my_list)):
                str = my_list[i]
                label = Label(displayvetsframe, text = str, font = "times 15")
