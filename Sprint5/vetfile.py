...
class vet():
    ...
    def vet_upload_pet_record_clicked(self):
        self.window.title("Upload Pet Record")
        self.upload_pet_rec()
        self.show_frame(self.vet_upload_pet_rec)
    ...
    def vet_daily_app(self):
        ...
        cursor.execute("SELECT VetID FROM VetAccountInfo INNER JOIN VetLoginInfo ON VetAccountInfo.VetLoginID = VetLoginInfo.VetLoginID WHERE VetAccountInfo.VetLoginID = ?", vet_login_ID)
        vet_id = cursor.fetchone()
        my_data = cursor.execute("select Concat('Time: ', AppointmentInfo.Time, ' Pet: ', PetInfo.PetName) from AppointmentInfo join PetInfo on AppointmentInfo.PetID = PetInfo.PetID where AppointmentInfo.Date = ? and AppointmentInfo.VetID = ?", currentdate, vet_id[0])
        ...
        if(len(my_list) == 0):
            ...
            buttonframe = Frame(self.vet_daily_appointment)
            buttonframe.pack(fill="x", anchor="se")
            upload_button = Button(buttonframe, text="Upload Pet Record",width=20, height=1, font='times 15', bd=20, bg='SpringGreen4', command=self.vet_upload_pet_record_clicked)
            upload_button.grid(row = 5, column = 5)
...
###########################################################################################################################################################################
    ...
    def vet_canceled_app(self):
        ...
        cursor.execute("SELECT VetID FROM VetAccountInfo INNER JOIN VetLoginInfo ON VetAccountInfo.VetLoginID = VetLoginInfo.VetLoginID WHERE VetAccountInfo.VetLoginID = ?", vet_login_ID)
        vet_id = cursor.fetchone()
        my_data = cursor.execute("""select Concat('Date: ', AppointmentInfo.Date, ' Time: ', AppointmentInfo.Time, ' Patient: ', PetInfo.PetName, ' Reason: ', CancellationInfo.CancelReason) 
                                    from CancellationInfo 
                                    join AppointmentInfo on AppointmentInfo.AppointmentID = CancellationInfo.AppointmentID 
                                    join PetInfo on PetInfo.PetID = AppointmentInfo.PetID 
                                    where AppointmentInfo.Cancelled = 1 and AppointmentInfo.VetID = ?""", vet_id[0])
        my_list = [r for r, in my_data]
    ...
    # This function allows vets to upload pet records
    def upload_pet_rec(self):
        ...
        cursor.execute("SELECT VetID FROM VetAccountInfo INNER JOIN VetLoginInfo ON VetAccountInfo.VetLoginID = VetLoginInfo.VetLoginID WHERE VetAccountInfo.VetLoginID = ?", vet_login_ID)
        vet_id = cursor.fetchone()
        my_data = cursor.execute("""SELECT Concat(PetWeight, ', ', PetDOB, ', ', PetMedsUsed, ', ', PetNotes, ', ', PetOutcome, ', ', PetRecom) 
                                    FROM PetRecords WHERE PetID = ?""", pet_id[1]) # SQLAlchem engine result
        
        my_list = [r for r, in my_data] # create a list
        sel = my_list[0].split(", ")

        if(len(my_list) == 0):
            label = Label(labelX, text = 'No pet record found.', font = "times 15")
            label.grid(row=0, column = 1)
            ...
            for i in range(len(my_list)):
                ...
                while (j < y):
                    label = Label(labelX, text = list_2[x], font = "times 15")
                    label.grid(row=j, column = 1)
                    str = list_2[0]
                    x+=1
                    j=j+1
                Button(labelX, text="Schedule Appointment", width=20, height=1, font='times 5', bd=20, bg='SpringGreen4', command = lambda vet_default = str : defaultvet(vet_default)).grid(row=j-1, column=1)
        ...
        pet_weight_label.grid(row=1, column=0, padx=5, pady=5)
        pet_weight_entry.grid(row=1, column=1, padx=5, pady=5)

        pet_meds_label = Label(labelX, text='Medication used:', font="times 15")
        pet_meds_entry = Entry(labelX, textvariable=pet_meds_var, font="times 15")
        pet_meds_entry.insert(0, sel[2])
        pet_meds_label.grid(row=2, column=0, padx=5, pady=5)
        pet_meds_entry.grid(row=2, column=1, padx=5, pady=5)

        pet_notes_label = Label(labelX, text='Notes:', font="times 15")
        pet_notes_entry = Text(labelX, font="times 15", height="10", width="40")
        pet_notes_entry.insert(0, sel[3])
        pet_notes_label.grid(row=2, column=7, padx=5, pady=5)
        pet_notes_entry.grid(row=2, column=8, padx=5, pady=5)
        ...
        pet_outcome_entry.insert(0, sel[4])
        pet_outcome_label.grid(row=4, column=1, padx=5, pady=5)
        pet_outcome_entry.grid(row=4, column=2, padx=5, pady=5)

        pet_recom_label = Label(labelX, text='Recommendations', font="times 15")
        pet_recom_entry = Entry(labelX, textvariable=pet_rec_steps_var, font="times 15")
        pet_recom_entry.insert(0, sel[5])
        pet_recom_label.grid(row=5, column=1, padx=5, pady=5)
        pet_recom_entry.grid(row=5, column=1, padx=5, pady=5)

        pet_dob_label = Label(labelX, text="Pet's Date of Birth", font = "times 15")
        pet_weight_entry.insert(0, sel[1])
        pet_dob_label.grid(row=6, column=1, padx=5, pady=5)

        month_label = Label(labelX, text="Month", font="times 15")
        month_option = OptionMenu(labelX, pet_month_var, month)
        month_label.grid(row=6, column=2, padx=5, pady=5)
        month_option.grid(row=7, column=2, padx=5, pady=5)

        day_label = Label(labelX, text="Day", font="times 15")
        day_option = OptionMenu(labelX, pet_day_var, day)
        day_label.grid(row=6, column=3, padx=5, pady=5)
        day_option.grid(row=7, column=3, padx=5, pady=5)

        year_label = Label(labelX, text="Year", font="times 15")
        year_option = OptionMenu(labelX, pet_year_var, year_list)
        year_label.grid(row=6, column=4, padx=5, pady=5)
        year_option.grid(row=7, column=4, padx=5, pady=5)

        butt = Button(labelX, text="Upload", width=20, height=1, font='times 15', bd=20, bg='SpringGreen4', command = self.upload_clicked)
        butt.grid(row = 8, column = 2)

    def upload_clicked(self):
        pet_weight_info = pet_weight_var.get()
        pet_meds_info = pet_meds_var.get()
        pet_notes_info = pet_notes_var.get()
        pet_outcome_info = pet_outcome_var.get()
        pet_rec_info = pet_rec_steps_var.get()
        pet_month_info = pet_month_var.get()
        pet_day_info = pet_day_var.get()
        pet_year_info = pet_year_var.get()
        pet_record_id = None
    
        cursor.execute("SELECT PetID FROM AppointmentInfo INNER JOIN PetRecords ON AppointmentInfo.PetID = PetRecords.PetID WHERE AppointmentInfo.VetID = ?", vet_id[0])
        cursor.commit()

        # global reason
        # reason = appoint_desc.get('1.0','end-1c')

        string_dob = f"{pet_month_info} {pet_day_info} {pet_year_info}"

        # if (pet_weight_info == "" or pet_meds_info == "" or 
        #     pet_notes_info == "" or pet_outcome_info == "" or
        #     pet_rec_info == "" or pet_month_info == "" or
        #     pet_day_info == "" or pet_year_info == ""):
        #     pet_weight_entry.delete(0, END)
        #     pet_meds_entry2.delete(0, END)
        #     pet_notes_entry2.delete(0, END)
        #     pet_recom_entry2.delete(0, END)
        #     u.empty_login_info()
        # else:
        #     cursor.execute("INSERT INTO PetRecords (PetWeight, PetDOB, PetMedsUsed, PetNotes, PetOutcome, PetRecom) VALUES (?, ?, ?, ?, ?, ?)", pet_weight_info, string_dob, pet_meds_info, pet_notes_info, pet_outcome_info, pet_rec_info)
