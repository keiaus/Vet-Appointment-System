...
# user class, contains all functions related to user actions
class user():

    # initializer for all included frames for the user 
    def __init__(self, window, account_page, account_create, user_log_in, user_pet_menu, user_pet_add, user_menu, 
            ...
            user_calendar_frame, cancel_menu_frame, user_searchvet_frame, user_pet_record):
        super().__init__()
        self.window = window
    ...
    # function to show frame when user clicks on add a pet, also deletes a label
    ...
    def user_update_pet_button_clicked(self):
        ...
        for widgets in self.user_pet_record.winfo_children():
                widgets.destroy()
        self.user_pet_record.pack_forget()
        self.window.title("Update Pet Information")
        self.user_update_pet_menu()
        self.show_frame(self.user_update_pet_info)
    
    # function to show frame when user clicks schedule appointment
    ...
    # function to clear all widgets from frames when called
    ...
    # function to show frame when user logs out of user menu  
    ...
    # creation of after successful user login menu widgets
    def user_after_login_menu(self):
        ...
        # all buttons leading to clicked functions
    ...
    # Creates all widgets that appear on the registration of a new account frame
    ...
    # Function to add information entered on the registration frame into the database
    ...
    # Function that tells the user that the username they're trying to register with was taken
    ...
    # Function that creates the widgets for the user log in frame
    def user_login(self):
    ...
    # Function for user login verification (checks that the entered user information matches a record in the UserLoginInfo table)
    ...
    # Function that retrieves userID of verified user login from the database
    def getUserID(self):
        user_login_ID = user.getUserLoginID(self)
    ...
    # Function that retrieves userLoginID of verified user login from the database
    ...
    # Function that tells the user that their login was invalid (whatever they entered didn't match anything in the UserLoginInfo table)
    ...
    # Function that removes the user_invalid_login() pop-up on click
    ...
    def user_update_pet_menu(self):
        ...
        Label(self.user_update_pet_info, text="").pack()
        # Button(self.user_update_pet_info, text='View Pet Records', width=20, height=1, font="times 15", bd=20, bg="SpringGreen4", command = self.pet_user_records).pack()
        # Label(self.user_update_pet_info, text="").pack()
    ...
    # function that creates a pop up confirming the user's selected choice of pet to be updated
    ...
    # Function that updates pet information based on user inputs from the user_update_pet_info frame
    ...
    # Function that destroys the pop up frame that confirmed the pet update information
    def pet_confirmation_des(self):
        pet_confirmation.destroy()

    # function that creates a pop up confirming the user's selected choice of pet to be deleted
    ...
    # Function that deletes pet information based on user inputs from the user_update_pet_info frame
    ...
    # Function that destroys the pop up frame that confirmed the pet delete information
    def pet_del_confirmation_des(self):
        pet_del_confirmation.destroy()
    
    # function to show all current stored records for selected pet
    # def pet_user_records(self):
    #     self.show_frame(self.user_pet_record)
    #     Label(self.user_pet_record, text='Pet Records', font="times 15").grid(row=0, column = 5)
    
    #     frame_canvas = tk.Frame(self.user_pet_record)
    #     frame_canvas.grid(row=1, column=5, pady=(5, 0), sticky='nw')
    #     frame_canvas.grid_rowconfigure(0, weight=1)
    #     frame_canvas.grid_columnconfigure(0, weight=1)
        
    #     canvas=Canvas(frame_canvas, width=1200, height=800)
    #     canvas.grid(row=0, column=0, sticky="news")
    #     vsb = tk.Scrollbar(frame_canvas, orient="vertical", command=canvas.yview)
    #     vsb.grid(row=0, column=1, sticky='ns')
    #     canvas.configure(yscrollcommand=vsb.set)

    #     Button(self.user_pet_record, text="Return to Manage Pets", width=20, height=1, font='times 15', bd=20, bg='SpringGreen4', command= self.user_update_pet_button_clicked).grid(row=2, column = 5)

    #     self.user_pet_record.grid_columnconfigure(0, weight=1)
    #     self.user_pet_record.grid_columnconfigure(6, weight=1)
        ####################################################################################################################################################################
    ...
    # Function to create the widgets for the Pet registration menu
    ...
    # Function to insert the pet information from the user_pet_add frame into the database
    def register_pet(self):
        ...
        if(pet_name_info == "" or pet_type_info == "" or pet_breed_info == "" or pet_color_info == ""):
            ...
            cursor.execute("SELECT UserID FROM UserAccountInfo INNER JOIN UserLoginInfo ON UserAccountInfo.UserLoginID = UserLoginInfo.UserLoginID WHERE UserAccountInfo.UserLoginID = ?", user_login_ID)
            user_id = cursor.fetchone()
            cursor.execute("INSERT INTO PetInfo VALUES(?,?,?,?,?,?,?)", pet_name_info, pet_type_info, pet_breed_info, pet_color_info, user_id[0], 0, 0)
    ...
    # Function that creates the user schedule submenu widgets
    def user_schedule_submenu(self):
        ...
        # get the current year and month for the starting date on the calendar (auto updates current year/month)
        current_year = datetime.today().year
        current_month = datetime.today().month

        # restrict the calendar so only dates from the current date and on can be selected from
        cal_mindate = datetime.today()
        
        cal = Calendar(calendar_display_frame, selectmode = 'day', year = current_year, month = current_month, mindate = cal_mindate)
        ...
        # function that retrieves the vet schedule to display on the calendar_display_frame
        def schedule_retrieval(event):
            ...
            # section of function that retrieves vet information from database to display on calendar_display_frame
        ...
        # binds the combo box in order to track what vet is selected    
        vetsel.bind("<<ComboboxSelected>>", schedule_retrieval)
        
        # creates widgets in order to select the pets which the user has
        ...
        # function that creates the time slot buttons when the Generate Time Slots button is pressed on calandar_display_frame
        def createScheduleButtons():
            ...
            for i in range(time_interval):
                ...
                cursor.execute("SELECT Date, Time FROM APPOINTMENTINFO WHERE Date = ? AND Time = ?",date_str, strung)
                cursor.commit()
        ...
        # function that creates appointment pop up confirming all enter information
        ...
        # function that destroys the appointment pop up
        def destroyAppointmentPopup():
            appointmentPopUp.destroy()
        
        # function that sends in all information to     
    ...
    # function that creates the cancel appointment menu widgets     
    def cancel_appointment(self):
        ...
        # function that updates an appointment with a boolean value in order to indicate cancelled
        def cancelApptQuery():
            global label
